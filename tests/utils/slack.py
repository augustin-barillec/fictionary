import logging
from utils import slack_api

logger = logging.getLogger(__name__)


def list_channels(slack_client):
    resp = slack_api.conversations_list(slack_client)
...
def list_members(slack_client, channel_id):
    resp = slack_api.conversations_members(
...
def create_channel(slack_client, channel_name):
    ...
    resp = slack_api.conversations_create(
...
def kick(slack_client, channel_id, user_ids):
    ...
    for user_id in user_ids:
        ...
        slack_api.conversations_kick(slack_client, channel_id, user_id)
...
def invite(slack_client, channel_id, user_ids):
    ...
    for user_id in user_ids:
        ...
        slack_api.conversations_invite(slack_client, channel_id, user_id)
...
def join(slack_client, channel_id):
    ...
    slack_api.conversations_join(slack_client, channel_id)
    logger.info(f'Ended join {channel_id}')


def leave(slack_client, channel_id):
    logger.info('Starting leave...')
    slack_api.conversations_leave(slack_client, channel_id)
