describe('main', () => {
  it('main', () => {
    cy.get_conf().then((conf) => {
      cy.get_channel_id('french_exception_setup_submission_max_running').then((channel_id) => {
        ...
        cy.contains(`${tag1}: Réponse: truth`)
        cy.contains(`${tag1}: Il y a déjà 4 parties en cours. C'est le nombre maximal autorisé.`)
