describe('main', () => {
  ...
  it('main', () => {
    ...
    cy.get_conf().then((conf) => {
      cy.get_channel_id('ending_zero').then((channel_id) => {
        ...
        cy.contains(`${tag}: • @augustin3: 3) g3`)
        cy.contains(`${tag}: • @augustin1 -> @augustin3`)
        cy.contains(`${tag}: • @augustin2 -> @augustin3`)
        cy.contains(`${tag}: • @augustin1: 0 points`)
        cy.contains(`${tag}: • @augustin2: 0 points`)
        cy.contains(`${tag}: • @augustin3: 0 points`)
